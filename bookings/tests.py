...
class RoomBookingViewTest(TestCase):
    ...
    def test_booking_page_authenticated(self):
        ...
        self.assertNotContains(
            response, 
            "Please <a href=\"/login/\">sign in</a> to book a room."
        )
    ...
    def test_booking_page_with_form_errors(self):
        ...
        response = self.client.post(
            self.url, {'room': '', 'start_time': '', 'end_time': ''}
        )
        self.assertEqual(response.status_code, 200)
...
class BookingTestCase(TestCase):
    ...
    def test_update_booking(self):
        ...
        response = self.client.post(
            reverse('change_booking', kwargs={'pk': booking.pk}),
            {
                'room': self.room.pk,
        ...
        self.assertEqual(
            updated_booking.end_time.strftime('%Y-%m-%d %H:%M:%S'), 
            new_end_time
        )
    ...
    def test_delete_booking(self):
        ...
        response = self.client.post(
            reverse('delete_booking', kwargs={'pk': booking.pk})
        )
        self.assertEqual(response.status_code, 302)

        # check if booking was deleted from db
        booking_exists = Booking.objects.filter(pk=booking.pk).exists()
        self.assertFalse(
            booking_exists,
            f"Booking with pk={booking.pk} still exists in the database."
        )
